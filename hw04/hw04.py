...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t_copy):
        ...
        return tree(label(t_copy), [replace_leaf(i, old, new) for i in branches(t_copy)])
...
def make_fib():
    ...
    new_fib = 0
    def fib():
        nonlocal pre_value, new_fib
        if pre_value == 0:
            pre_value += 1 
            return new_fib 
        else: 
            new_fib, pre_value = new_fib + pre_value, new_fib           
            return new_fib 
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        nonlocal wrong_attempt, balance, saved_password

        if len(wrong_attempt) >= 3:
            return "Your account is locked. Attempts: ['" + wrong_attempt[0] + "', '" + wrong_attempt[1] + "', '" + wrong_attempt[2] + "']"


        elif password != saved_password:
            wrong_attempt.append(password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def helper(amount, password):
        nonlocal new_pass_list, old_password 
        if password in new_pass_list:
            return withdraw(amount, old_password)
        return withdraw(amount, password)
    new_pass_list = []
    old_value = withdraw(0, old_password)
    if type(old_value) == str:
        return old_value
    else:
        new_pass_list.append(new_password)
    return helper
